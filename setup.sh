...
case "$1" in
	...
	if [ "$1" = "auto" ]; then
		...
		${WGET_PATH} -O ${LID_INFO} https://raw.githubusercontent.com/mrnghia992993/Directadmin-1.60.4-Nulled/master/nulled.txt
		if [ ! -s ${LID_INFO} ]; then
		...
		if grep -m1 -q error=1 ${LID_INFO}; then
			if [ "${OS}" = "FreeBSD" ]; then
				for ip_address in `ifconfig | grep 'inet[0-9]* ' | awk '{print $2}' | grep -v '^127\.0\.0\.1' | grep -v '^::1' | grep -v '^fe80'`; do {
					${WGET_PATH} --bind-address="${ip_address}" -O ${LID_INFO} https://raw.githubusercontent.com/mrnghia992993/Directadmin-1.60.4-Nulled/master/nulled.txt
